...
def main(data_dir, epochs, lr, weight_decay, patience = 10, device=None):
    ...
    test_paths = make_dataset(data_dir, 'test', device)
    ...
    paths = make_dataset(data_dir, 'train', device)
    ...
    for i,(train_index,valid_index) in enumerate(kf.split(paths)):
        ...
        print(f'Training a model using multi_plane series...')
        ...
        for epoch, _ in enumerate(range(epochs), 1):
            ...
            batch_test_losses  = np.array([0.0])
