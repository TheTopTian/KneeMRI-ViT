...
def batch_forward_backprop(models, inputs, labels, criterions, optimizers):
    ...
    out = models(inputs[0],inputs[1],inputs[2])
    # print(f"out: {out.shape}")
    # print(f"labels: {labels.shape}")
    loss = criterions(out, labels.squeeze(0))
    # print(f"loss: {loss}")
    loss.backward()
    optimizers.step()
...
def main(data_dir, epochs, lr, weight_decay, patience = 5, device=None):

    exp = f'{datetime.now():%Y-%m-%d_%H-%M}'
    out_dir, auc_path = create_auc_dir(exp)
    ...
    labels_path = f'../../../new_location/json_for_new/new_grouped_whatever_new.csv'
    paths = make_dataset(data_dir, labels_path, device)
    kf = KFold(n_splits=5, shuffle = False)

    for i,(train_index,valid_index) in enumerate(kf.split(paths)):
        Fold_num = i
        losses_path = create_loss_dir(out_dir, Fold_num)
        ...
        print('Training a model using multi series...')
        ...
        for epoch, _ in enumerate(range(epochs), 1):
            ...
            for inputs, labels in train_loader:
                inputs, labels = (inputs[0].to(device),inputs[1].to(device),inputs[2].to(device)), labels.to(device)
            ...
            if batch_valid_losses[0] < min_valid_losses[0]:
                save_checkpoint(Fold_num, epoch, models,
                                optimizers, out_dir)
