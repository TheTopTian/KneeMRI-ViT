...
location_csv = pd.read_csv("../../../new_location/json_for_new/new_grouped_whatever_new.csv")
...
def create_auc_dir(exp):
    out_dir = f'./models/{exp}'
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)

    auc_path = create_auc_csv(out_dir)

    return out_dir, auc_path

def create_loss_dir(out_dir, Fold_num):
    losses_path = create_losses_csv(out_dir, Fold_num)

    return losses_path


def create_auc_csv(out_dir):
    auc_path = f'{out_dir}/highest_auc.csv'
...
def create_losses_csv(out_dir, Fold_num):
    losses_path = f'{out_dir}/Fold{Fold_num}_losses_multi.csv'
...
def save_checkpoint(Fold_num, epoch, model, optimizer, out_dir):
    ...
    chkpt = f'MultiNet_Fold{Fold_num}_multi.pt'
...
def add_mean_auc(auc_path):
    ...
    df_new.to_csv(f"{dir_path}/mean_auc_multi.csv",index=False)