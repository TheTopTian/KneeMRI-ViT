...
location_csv = pd.read_csv("../../new_data/ACL.csv")
column_name = ["ACL"]
# column_name = location_csv.columns.tolist()[1:]

def preprocess_data(case_path,predict_slice, transform=None):
    img = sitk.ReadImage(case_path)
    series = sitk.GetArrayFromImage(img).astype(np.float32)[predict_slice,:,:].squeeze()
    series = zoom(series, (0.875,0.875)) # shrimp to (448,448)
...
def save_checkpoint(Fold_num, epoch, plane, model, optimizer, out_dir):
    ...
    chkpt = f'DINOV2_Fold{Fold_num}_{plane}.pt'
