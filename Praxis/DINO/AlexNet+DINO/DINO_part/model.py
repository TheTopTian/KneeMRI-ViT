...
class VisionTransformer(nn.Module):
    ...
    def interpolate_pos_encoding(self, x, w, h):
        ...
        N = self.pos_embed.shape[1] - 1 # N=784
        # self.pos_embed.shape: [1,785,768]
        if npatch == N and w == h:
            return self.pos_embed
        class_pos_embed = self.pos_embed[:, 0] # [1, 768]
        patch_pos_embed = self.pos_embed[:, 1:] # [1, 784, 768]
        dim = x.shape[-1] # 768
        # patch_size = 8
        w0 = w // self.patch_embed.patch_size # 512/8=64
...
class VitGenerator(object):
    def __init__(self, name_model, patch_size, device, evaluate=True, random=False, verbose=False, checkpoint=None):
        ...
        self.model,self.embed_dim = self._getModel()
        self.linear = nn.Linear(self.embed_dim,1).to(self.device)
        if not random:
    ...
    def _getModel(self):
        ...
        elif self.name_model == 'dinov2_vitb14':
            model = VisionTransformer(patch_size=self.patch_size, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4,
                                      qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6))
            embed_dim = 768
    ...
    def _loadPretrainedWeights(self):
        ...
        if self.name_model == 'vit_base' and self.patch_size == 8 and self.evaluate == True:
            checkpoint = torch.load(f'./models/{self.checkpoint}/DINO_Fold0_sagittal.pt')
            self.model.load_state_dict(checkpoint['state_dict'], strict=True)
            return
        
        if self.name_model == 'dinov2_vitb14':
        ...
        url = None
        if self.name_model == 'vit_small' and self.patch_size == 16 and self.evaluate == False:
            url = "dino_deitsmall16_pretrain/dino_deitsmall16_pretrain.pth"

        elif self.name_model == 'vit_small' and self.patch_size == 8 and self.evaluate == False:
            url = "dino_deitsmall8_300ep_pretrain/dino_deitsmall8_300ep_pretrain.pth"

        elif self.name_model == 'vit_base' and self.patch_size == 16 and self.evaluate == False:
            url = "dino_vitbase16_pretrain/dino_vitbase16_pretrain.pth"

        elif self.name_model == 'vit_base' and self.patch_size == 8 and self.evaluate == False:
    ...
    def get_first_selfattention(self, img):
        return self.model.get_first_selfattention(img.to(self.device))
...
if __name__ == "__main__":
    ...
    img = img_array[15]
    img = torch.tensor(np.stack((img,)*3, axis=0))
    y = model(img.unsqueeze(0).to(device))
    visualize_predict(model, img, patch_size, device)