...
def get_attention(attentions):
    ...
    normalized_attention = F.sigmoid(mean_attention)
...
def batch_forward_backprop(models, inputs, labels, optimizers, criterions):
    ...
    if labels.shape != (1,):
        ...
        # weights = torch.tensor(labels > 0, dtype=torch.float32) * 16500 + 1 # 512*512*32/13/13/3 = 16545
        attentions = get_attention(attn)
        loss2 = torch.mean(torch.square(labels -attentions))
        loss = loss1 + loss2
    else: 
...
def batch_forward(models, inputs, labels, criterions):
    ...
    if labels.shape != (1,):
        loss1 = criterions(out.squeeze(0), torch.tensor([1.0]).to("cuda"))
        valid_labels = [1.0]
        # weights = torch.tensor(labels > 0, dtype=torch.float32) * 16500 + 1
        attentions = get_attention(attn)
        loss2 = torch.mean(torch.square(labels - attentions))
        loss = loss1 + loss2
    else:
...
def main(data_dir, plane, disease_num, epochs, lr, weight_decay, patience=10, device=None):
    ...
    exp = f'disease{disease_num}_overlap_label'
